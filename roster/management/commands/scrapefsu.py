...
class Command(BaseCommand):
	args = '<url>'
	help = 'Parses and imports player info from the fsu athletic department website'
	...
	def handle(self, *args, **options):
		try:
			...
			response = urllib2.urlopen("http://www.seminoles.com/sports/m-baskbl/mtt/fsu-m-baskbl-mtt.html")
			html = response.read()
			...
			for player_link, val in enumerate(player_links):
				...
				response = urllib2.urlopen('http://seminoles.com%s' % (val), val)
				html = response.read()
		...
		self.stdout.write('end of scrapefsu.py')