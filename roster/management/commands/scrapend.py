...
class Command(BaseCommand):
	args = '<url>'
	help = 'Parses and imports player info from the notre dame athletic department website'
	...
	def handle(self, *args, **options):
		try:
			...
			response = urllib2.urlopen("http://www.und.com/sports/m-baskbl/mtt/nd-m-baskbl-mtt.html")
			html = response.read()
			...
			current_team.twitter = 'FightingIrish'
			current_team.color = '#002B5B'
			current_team.logo = 'logos/notredame.png'
			current_team.portrait = 'portraits/teams/edmundjoycecenter.jpg'
			current_team.save()
			...
			for player_link, val in enumerate(player_links):
				...
				response = urllib2.urlopen('http://und.com%s' % (val), val)
				html = response.read()
		...
		self.stdout.write('end of scrapend.py')